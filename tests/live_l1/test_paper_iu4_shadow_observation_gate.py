...
class PaperIU4ShadowObservationGateTests(unittest.TestCase):
    ...
    def test_full_history_hard_boundary_is_exact_and_still_requires_explicit_bound(self) -> None:
        environment = dict(self.environment)
        environment[ENV_OBSERVATION_MAX_RECORDS] = "1042658"
        gate = self._evaluate(environment=environment, max_ticks=1_042_658)
        try:
            self.assertTrue(gate.enabled)
            self.assertEqual(gate.max_records, 1_042_658)
            assert gate.observer is not None
            checkpoint = json.loads(
                self.evidence_path.read_text(encoding="utf-8")
            )
            self.assertEqual(checkpoint["max_records"], 1_042_658)
        ...
        environment[ENV_OBSERVATION_MAX_RECORDS] = "1042659"
        self.evidence_path.unlink()
        self.records_journal_path.unlink()
        with self.assertRaises(IU4ShadowObservationError) as excessive:
            self._evaluate(environment=environment, max_ticks=1_042_659)
        self.assertEqual(
