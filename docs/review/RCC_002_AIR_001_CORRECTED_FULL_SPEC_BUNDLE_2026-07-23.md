# RCC-002 AIR-001 Corrected Full Specification Bundle

Enthaltene Spezifikationen: 7

Korrekturstand: AIR-001-Corrected Draft – Focused Scientific Re-Review Pending
...
| Version | 0.6.0 |
| Datum | 2026-07-23 |
| Status | AIR-001-Corrected Draft – Focused Scientific Re-Review Pending |
...
Jede Änderung dieses Hashes erzeugt einen neuen `build_id`. Wenn sich dadurch
der veröffentlichte semantische Datasetinhalt ändert, entsteht auch ein neuer
`dataset_id`.
...
Eine reine Repartitionierung oder Neuverpackung mit semantisch identischem
Inhalt behält `build_id` und `dataset_id`, erzeugt aber neue physische
Artefaktidentitäten und eine neue `dataset_artifact_set_id`.
...
Folgende logische Schema-IDs sind für die erste RCC-002-Baseline reserviert:

| Stufe | Akzeptierter Eingang | Erzeugter Ausgang |
|---|---|---|
| `S0_SOURCE` | externe unveränderte Quellbytes und Provider-Metadaten | `rcc002.stage.s0-source/1.0.0` |
| `S1_NORMALIZED` | `rcc002.stage.s0-source/1.0.0` | `rcc002.stage.s1-normalized/1.0.0` |
| `S2_VALIDATED` | `rcc002.stage.s1-normalized/1.0.0` | `rcc002.stage.s2-validated/1.0.0` |
| `S3_INDICATORS` | `rcc002.stage.s2-validated/1.0.0` | `rcc002.stage.s3-indicators/1.0.0` |
| `S4_SIGNALS` | `rcc002.stage.s3-indicators/1.0.0` | `rcc002.stage.s4-signals/1.0.0` |
| `S5_REGIMES` | `rcc002.stage.s4-signals/1.0.0` | `rcc002.stage.s5-regimes/1.0.0` |
| `S6_GATES` | `rcc002.stage.s5-regimes/1.0.0` | `rcc002.stage.s6-gates/1.0.0` |
| `S7_LABELS` | `rcc002.stage.s6-gates/1.0.0` | `rcc002.stage.s7-labels/1.0.0` |
| `S8_EXPORT` | freigegebene Kombination registrierter Stufenschemas | registriertes S8-View-Schema |
...
S0 enthält unveränderte Quellartefakte und das zugehörige Source Manifest.
...
Der kanonische S0-Provenienzvertrag lautet:
...
| `provider` | UTF-8-String | Nein | Kanonische Providerkennung |
| `market_type` | UTF-8-String | Nein | Registrierter Markttyp |
| `symbol` | UTF-8-String | Nein | Registriertes Symbol |
| `interval` | UTF-8-String | Nein | Registriertes Datenintervall |
...
| `source_format` | UTF-8-String | Nein | Registrierte Format- und Schemafamilie |
...
`provider` ist der einzige kanonische Providerfeldname.
`source_provider` ist kein zusätzliches kanonisches Feld.

`retrieved_at_utc` ist der einzige kanonische Abrufzeitfeldname.
`source_retrieved_at_utc` ist kein zusätzliches kanonisches Feld.

Frühere Aliasnamen dürfen nur in einer versionierten S1-Migrationsabbildung
akzeptiert werden. Im kanonischen S0- oder S1-Ausgang dürfen sie nicht parallel
...
`data_gate_pass` in S6 übernimmt für die aktuelle Zeile zunächst exakt
`quality_gate_pass` und ergänzt ausschließlich die in der
Regime-/Gate-Spezifikation definierten Schlüssel-, Segment- und
Schema-Konsistenzprüfungen.
...
| View-Schema-ID | Zulässige Erzeugerstufen | S7 zulässig |
|---|---|:---:|
| `rcc002.view.research-features/1.0.0` | S0 bis S6 | Nein |
| `rcc002.view.backtest-inputs/1.0.0` | S0 bis S6 | Nein |
| `rcc002.view.paper/1.0.0` | S0 bis S6 | Nein |
| `rcc002.view.live/1.0.0` | S0 bis S6 | Nein |
| `rcc002.view.label-research/1.0.0` | registrierte Felder aus S0 bis S7 | Ja |
| `rcc002.view.audit/1.0.0` | registrierte Felder und Manifeste aus S0 bis S8 | Ja |

Die konkrete positive Feld-Allowlist jeder View ist Bestandteil ihres
versionierten logischen Schemas.

Sie muss jedes zulässige Feld einzeln aufzählen. Wildcards, ausschließlich
präfixbasierte Freigaben oder die pauschale Freigabe einer unbekannten
Schemaerweiterung sind unzulässig.

Paper und Live müssen für den gemeinsamen Feature-, Signal-, Regime- und
Gate-Kern dieselbe Feld-Allowlist besitzen.

Backtest- und Research-Feature-Views enthalten keine S7-Felder. Für die
Outcome-Auswertung werden sie über den kanonischen Primärschlüssel mit einer
ausdrücklich ausgewählten Label-Research-View verbunden.

Live-, Paper-, Backtest-Input- und Research-Feature-Views müssen sämtliche
Felder ausschließen, deren Erzeugerstufe S7 ist.

Der Ausschluss wird primär über die im Schema registrierte Erzeugerstufe und
die positive View-Allowlist geprüft.

Zusätzlich müssen Leakage-Tests alle Felder mit folgenden Präfixen ablehnen:
...
Ein unbekanntes Feld oder ein Feld ohne registrierte Erzeugerstufe ist für
sämtliche S8-Views fail-closed abzulehnen.
...
Version 0.6.0 korrigiert die diesem Dokument zugeordneten Teile von:

- `AIR-001-B01` – kanonischer S0–S2-Provenienz-, Qualitäts- und
  Segmentvertrag;
- `AIR-001-B02` – einheitlicher S7-Namensraum, Horizon-Register und
  S8-Leakageschutz;
- `AIR-001-B03` – einheitliche S5-/S6-Zustands- und Ausgabeverträge;
- `AIR-001-M01` – kanonisches Stufen- und View-Schemaregister;
- `AIR-001-M02` – Trennung semantischer und physischer Konfiguration;
- `AIR-001-M03` – eindeutige Implementierungsfreigabegrenzen;
- `AIR-001-m01` – kanonische Dokument-ID
...
```text
AIR-001-Corrected Draft – Focused Scientific Re-Review Pending
```
...
| Datum | 2026-07-23 |
| Status | AIR-001-Corrected Draft – Focused Scientific Re-Review Pending |
| Übergeordnetes Dokument | `RCC_002_DATA_PIPELINE_SPECIFICATION_2026-07-23.md`, Version 0.6.0 |
...
| `timezone` | UTF-8-String | Nein | Nachweisbare Quellzeitzone; kanonisch später UTC |
...
S6 darf ausschließlich die in der Regime-/Gate-Spezifikation definierten:

- Schlüsselprüfungen;
- Segmentkonsistenzprüfungen;
- Schemakonsistenzprüfungen
...
15. `quality_gate_pass` für jede Zeile exakt nach Abschnitt 15.1 berechnet
    wurde;
16. jede veröffentlichte kanonische S2-Zeile
    `quality_gate_pass=true` besitzt;
...
Version 0.3.0 korrigiert die diesem Dokument zugeordneten Teile von:

- `AIR-001-B01` – einheitlicher S0–S2-Provenienz-, Qualitäts- und
  Segmentvertrag;
- `AIR-001-M01` – konkrete S0-, S1- und S2-Schema-IDs sowie Feld- und
  Kompatibilitätsverträge;
- `AIR-001-M03` – Trennung von Spezifikationsreife, Implementierungsabnahme
  und physischer Implementierungskonkretisierung.
...
| Status | AIR-001-Corrected Draft – Focused Scientific Re-Review Pending |
| Übergeordnetes Dokument | `RCC_002_DATA_PIPELINE_SPECIFICATION_2026-07-23.md`, Version 0.6.0 |
| Direkte Abhängigkeit | `RCC_002_DATA_VALIDATION_2026-07-23.md`, Version 0.3.0 |
...
- bei unkonsolidierten Multi-Provider-Daten zusätzlich `provider` im
  Schlüssel,
...
Im kanonischen S3-Publication-Build muss jede Eingabezeile
`quality_gate_pass=true` besitzen. Diagnoseberechnungen dürfen Zeilen mit
`quality_gate_pass=false` erhalten, müssen aber sämtliche davon abhängigen
Indikatorwerte als ungültig ausweisen und dürfen nicht kanonisch
veröffentlicht werden.
...
| `indicator_schema_id` | UTF-8-String | Nein | `rcc002.stage.s3-indicators/1.0.0` |
...
```text
indicator_state_schema_id=rcc002.state.s3-indicators/1.0.0
...
2. jede S2-Eingabezeile `quality_gate_pass=true` besitzt;
...
- `AIR-001-B01` – einheitlicher S2-Eingangs-, Qualitäts- und Segmentvertrag
  sowie eindeutige Abgrenzung von `market_segment_id` und
  `indicator_segment_id`;
- `AIR-001-M01` – konkrete S2-/S3-Schema-IDs, positive S3-Feldallowlist,
  Begleitfelder, Eigentümer- und Kompatibilitätsregeln;
- `AIR-001-M03` – Trennung von Spezifikationsreife, Implementierungsabnahme
  und physischer Implementierungskonkretisierung.
...
| Status | AIR-001-Corrected Draft – Focused Scientific Re-Review Pending |
| Übergeordnetes Dokument | `RCC_002_DATA_PIPELINE_SPECIFICATION_2026-07-23.md`, Version 0.6.0 |
| Direkte Abhängigkeiten | `RCC_002_DATA_VALIDATION_2026-07-23.md`, Version 0.3.0; `RCC_002_INDICATOR_SPECIFICATION_2026-07-23.md`, Version 0.3.0 |
...
- die vier verbindlichen Profil- und Schemametadaten.
...
- die Sortierung `(symbol, timeframe, open_time)` unverändert erhalten,
...
```text
symbol ASC, timeframe ASC, open_time ASC
```
...
- `AIR-001-B03` – eindeutige S4-Abgrenzung zum kanonischen S5-/S6-Vertrag;
- `AIR-001-M01` – konkrete S3-/S4-Schema-IDs, positive S4-Feldallowlist,
  Typen, Nullbarkeit, Eigentümer-, Rollen- und Kompatibilitätsregeln;
- `AIR-001-M03` – Trennung von Spezifikationsreife, Implementierungsabnahme
  und physischer Implementierungskonkretisierung.
...
| Status | AIR-001-Corrected Draft – Focused Scientific Re-Review Pending |
| Übergeordnetes Dokument | `RCC_002_DATA_PIPELINE_SPECIFICATION_2026-07-23.md`, Version 0.6.0 |
| Direkte Abhängigkeiten | `RCC_002_DATA_VALIDATION_2026-07-23.md`, Version 0.3.0; `RCC_002_INDICATOR_SPECIFICATION_2026-07-23.md`, Version 0.3.0; `RCC_002_SIGNAL_TRANSFORMATION_2026-07-23.md`, Version 0.3.0 |
...
Nur wenn `data_gate_pass` aufgrund eines Schema-, Feld- oder
Gültigkeitsfehlers nicht deterministisch gebildet werden kann, gilt:
...
1. Kann `data_gate_pass` nicht deterministisch gebildet werden, erhalten beide
   Listen `GATE_INPUT_INVALID`; die Auswertung endet mit `INVALID`.
2. Ist `data_gate_pass=false`, erhalten beide Listen ausschließlich
...
- deterministisches `data_gate_pass=false` → beide blockiert, `BLOCK_BOTH`,
  `gate_valid=true`;
- nicht deterministisch bildbares Daten-Gate → beide blockiert, `INVALID`,
  `gate_valid=false`.
...
Version 0.4.0 korrigiert die diesem Dokument zugeordneten Teile von:

- `AIR-001-B01` – einheitliche Verwendung des kanonischen S2-Qualitäts-Gates
  sowie unveränderte Markt- und Indikatorsegmentübergabe;
- `AIR-001-B03` – ein einziger kanonischer S5-/S6-Vertrag mit exakten
  Zuständen, Feldern, Gültigkeitsregeln und Reason-Code-Listen;
- `AIR-001-M01` – konkrete S4-/S5-/S6- und State-Schema-IDs, positive
  Feldallowlists, Typen, Nullbarkeit, Eigentümer- und Kompatibilitätsregeln;
- `AIR-001-M03` – Trennung von Spezifikationsreife, Implementierungsabnahme
  und physischer Implementierungskonkretisierung.
...
| Status | AIR-001-Corrected Draft – Focused Scientific Re-Review Pending |
| Übergeordnetes Dokument | `RCC_002_DATA_PIPELINE_SPECIFICATION_2026-07-23.md`, Version 0.6.0 |
| Direkte Abhängigkeiten | `RCC_002_DATA_VALIDATION_2026-07-23.md`, Version 0.3.0; `RCC_002_INDICATOR_SPECIFICATION_2026-07-23.md`, Version 0.3.0; `RCC_002_SIGNAL_TRANSFORMATION_2026-07-23.md`, Version 0.3.0; `RCC_002_REGIME_AND_GATE_SPECIFICATION_2026-07-23.md`, Version 0.4.0 |
...
- `AIR-001-B01` – kanonische S2-Qualitätsfelder,
  `market_segment_id`, Abgrenzung zu `indicator_segment_id` und verbindliche
  Zukunftsfensterregeln;
- `AIR-001-B02` – ein einziges Horizon-Register, ein einheitlicher
  S7-Namensraum, exakte S7-Schemaidentität, Feldprovenienz und vollständiger
  stufenbasierter Leakage-Schutz;
- `AIR-001-M01` – konkrete S6-/S7-Schema-IDs, Typen, Nullbarkeit,
  Feldschablonen, Eigentümerstufen, Kompatibilitäts- und
  S8-View-Schnittstellen;
- `AIR-001-M03` – Trennung von Spezifikationsreife,
  Implementierungsabnahme und physischer Implementierungskonkretisierung.
...
1. Reproducibility-and-Manifest-Spezifikation korrigieren;
2. vollständige interne Qualitätskontrolle;
3. neues vollständiges Spezifikationspaket;
4. fokussierter Scientific Consistency Re-Review;
5. fokussierter Architecture Integrity Re-Review;
...
| Primäre Abhängigkeit | `RCC_002_DATA_PIPELINE_SPECIFICATION_2026-07-23.md`, Version `0.6.0` |
| Fachliche Abhängigkeiten | `RCC_002_DATA_VALIDATION_2026-07-23.md`, Version `0.3.0`; `RCC_002_INDICATOR_SPECIFICATION_2026-07-23.md`, Version `0.3.0`; `RCC_002_SIGNAL_TRANSFORMATION_2026-07-23.md`, Version `0.3.0`; `RCC_002_REGIME_AND_GATE_SPECIFICATION_2026-07-23.md`, Version `0.4.0`; `RCC_002_LABEL_AND_FORWARD_RETURN_SPECIFICATION_2026-07-23.md`, Version `0.3.0` |
...
- fokussierter Scientific Consistency Re-Review der AIR-001-Korrekturen;
...
Der Hash MUSS `build_id` und dadurch den logischen Datasetinhalt beeinflussen.
...
Das Source Manifest MUSS die kanonischen S0-Provenienzfelder referenzieren:

| Feld | Bedeutung |
|---|---|
| `source_snapshot_id` | deterministische Identität der Quelldatenfassung |
| `source_provider` | kanonische Providerbezeichnung |
| `source_retrieved_at_utc` | tatsächlicher Abrufzeitpunkt, nur Provenienz |
...
Historische Aliasnamen wie `provider`, `retrieved_at_utc`,
`source_location` oder `source_format` dürfen nur in einer versionierten
Migrationsabbildung erscheinen. Sie sind keine parallelen kanonischen
S0-Felder.
...
- `build_id`;
- `run_id`;
- akzeptierte Eingangsschema-ID und Version;
- erzeugte Ausgangsschema-ID und Version;
- Eingangs- und Ausgangsschema-Fingerprint;
- Parent-Artefakte;
- Ausgabeartefakte;
- `semantic_build_configuration_sha256`;
...
| Manifesttyp | Manifest-Schema-ID |
|---|---|
| Source Manifest | `rcc002.source-manifest/1.0.0` |
| Stage Manifest | `rcc002.stage-manifest/1.0.0` |
| Run Manifest | `rcc002.run-manifest/1.0.0` |
| Dataset Manifest | `rcc002.dataset-manifest/1.0.0` |
| Review Manifest | `rcc002.review-manifest/1.0.0` |
| Reproduction Manifest | `rcc002.reproduction-manifest/1.0.0` |
...
View-Schema-ID referenzieren:

| View-Schema-ID | S7 zulässig |
|---|:---:|
| `rcc002.view.research-features/1.0.0` | Nein |
| `rcc002.view.backtest-inputs/1.0.0` | Nein |
| `rcc002.view.paper/1.0.0` | Nein |
| `rcc002.view.live/1.0.0` | Nein |
| `rcc002.view.label-research/1.0.0` | Ja |
| `rcc002.view.audit/1.0.0` | Ja |

Das Manifest referenziert für jede View:

- View-Schema-ID und Version;
...
Version `0.5.0` korrigiert zusätzlich:

- `AIR-001-B01` – kanonische Provenienz-, Qualitäts- und Segmentreferenzen;
- `AIR-001-B02` – einheitliche S7-Registry und stufenbasierten
  S8-Leakageschutz;
- `AIR-001-B03` – eindeutige S5-/S6-Identitäten;
- `AIR-001-M01` – konkrete Stufen- und View-Schemareferenzen;
- `AIR-001-M02` – getrennte semantische und physische
  Konfigurationsnamensräume;
- `AIR-001-M03` – einheitlichen Implementierungsfreigabezeitpunkt;
- `AIR-001-m01` – kanonische Data-Pipeline-Dokument-ID
  `RCC_002_DATA_PIPELINE_SPECIFICATION`.
...
1. vollständige interne Qualitätskontrolle aller sieben Spezifikationen;
2. neues vollständiges Spezifikationspaket;
3. fokussierter Scientific Consistency Re-Review;
4. fokussierter Architecture Integrity Re-Review;
5. Editorial Pass;
6. Internal Certification;
7. Claude Independent Architecture Review;
8. Gemini Independent Scientific and Adversarial Audit;
