# RCC-002 S8-RR-003 Normative Ledger Blocker Correction Proposal Revision 2

## Document control

| Field | Value |
|---|---|
| Proposal ID | `RCC-002-S8RR003-NLBCP-001-REV2` |
| Date | `2026-08-01` |
| Supersedes | `RCC_002_S8RR003_NORMATIVE_LEDGER_BLOCKER_CORRECTION_PROPOSAL_2026-08-01.md` |
| Controlling review | `RCC_002_S8RR003_NORMATIVE_LEDGER_BLOCKER_CORRECTION_PROPOSAL_INDEPENDENT_REVIEW_2026-08-01.md` |
| Review findings addressed | `IR-F01`, `IR-F02`, `IR-F03` |
| Repository baseline | `4e02828492873d1d0e65179ce495e9d9c0a48fff` |
...
| Status | Proposed for independent scientific and architecture re-review |
...
## 1.1 Revision 2 closure of independent-review findings

Revision 2 changes only the proposal contract. It does not generate the
five-file correction candidate.

| Finding | Disposition | Revision 2 correction |
|---|---|---|
| `IR-F01` | CLOSED | Section 5 item 3 now contains the literal intended regex `^[0-9a-f]{64}  \./[^\\\r\n]+$`. It requires a literal `./` prefix and excludes literal backslash plus actual CR and LF bytes. |
| `IR-F02` | CLOSED | Section 8 now adds explicit CRLF, missing-final-newline, extra-final-newline and non-ASCII mutations. |
| `IR-F03` | CLOSED | Section 4.3 now states that `scope_schema_version` is namespaced by `scope_id` and is not a global common-schema version. |
| `IR-F04` | NOT APPLICABLE | Git ancestry remains a controlled-import gate, as recommended. |
| `IR-F05` | NOT APPLICABLE | Protected-builder exclusion remains an explicit new literal path-membership gate. |

The six-path S8-RR-003 lifecycle set now names this Revision 2 proposal instead
of the superseded proposal. The set cardinality remains six and the exact union
remains 145 paths.
...
The superseded proposal and its rejecting independent review remain immutable
historical governance records in Git. They are not current normative inputs and
are not added to the successor root ledger. Revision 2 is the sole current
proposal path in the six-item lifecycle set. This replacement does not change
the set cardinality or the 145-path union.
...
8. the verifier rejects all 29 required mutations before reporting `PASS`;
