# RCC-002 SCR-002 Full Specification Bundle
...
| Dokument-ID | RCC-002 |
| Titel | Data Pipeline Specification |
...
| Version | 0.4.0 |
| Datum | 2026-07-23 |
| Status | SCR-Corrected Draft – Re-Review Pending |
...
| Editorial Pass | Ausstehend | Terminologie und redaktionelle Vereinheitlichung nach Fertigstellung der nachgeordneten Spezifikationen |
| Scientific Consistency Review | Nicht bestanden; zweite Korrektur eingearbeitet | SCR-B01 bis SCR-B03 aus `RCC-002-SCR-001` in Version 0.3.0 geschlossen; SCR-RR-B01 und SCR-RR-m01 aus `RCC-002-SCR-002` in Version 0.4.0 korrigiert; erneuter Re-Review ausstehend |
| Architecture Integrity Review | Ausstehend | Prüfung vor Implementierungsfreigabe |
...
RCC-002 soll aus überprüften Rohmarktdaten deterministisch, reproduzierbar und
ohne Look-ahead Leakage kanonische Forschungsdatensätze erzeugen. Diese
Datensätze bilden die gemeinsame Grundlage für:

- Signal- und Strategieforschung,
- Regime- und Gate-Analysen,
- Forward-Return- und Label-Bildung,
- Backtests und Walk-Forward-Verfahren,
- Paper-Trading- und Live-Paritätsprüfungen,
- spätere ML- und adaptive Steuerungsverfahren.

Dieses Dokument legt die Pipeline-Struktur fest. Die exakten Formeln,
Schwellenwerte und Versionen einzelner Indikatoren, Signaltransformationen,
Regime, Gates und Labels werden in nachgeordneten Spezifikationen definiert.

## 2. Normative Begriffe

Die Schlüsselwörter `MUST`, `MUST NOT`, `SHOULD`, `SHOULD NOT` und `MAY` sind
normativ:

- `MUST` / `MUST NOT`: verbindliche Anforderung.
- `SHOULD` / `SHOULD NOT`: begründete Standardentscheidung; Abweichung nur mit
  dokumentierter Rechtfertigung.
- `MAY`: zulässige, optionale Erweiterung.
...
1. Rohdatenaufnahme und Provenienz.
2. Schema- und Zeitachsen-Normalisierung.
3. Datenqualitätsprüfung.
4. Bildung kanonischer OHLCV-Reihen.
5. Indikatorberechnung.
6. Signaltransformation.
7. Marktregimeklassifikation.
8. getrennte Long-/Short-Handels-Gates.
9. Forward Returns und Labels.
10. Export, Manifest, Checksummen und Lineage.
11. Reproduzierbarkeits- und Paritätsprüfungen.

### 3.2 Nicht enthalten

Nicht Gegenstand dieses Dokuments sind:

- konkrete Strategieauswahl oder Strategiegewichtung,
- Orderausführung, Positionsgröße und Portfolioallokation,
- Backtest-Kennzahlen und Strategie-Ranking,
- Live-Betriebsführung,
- endgültige wissenschaftliche Freigabe einzelner Formeln.

Diese Verantwortlichkeiten dürfen RCC-002-Datenfelder konsumieren, aber die
kanonische Datenpipeline nicht implizit verändern.

## 4. Evidenzbasis und Legacy-Befunde

### 4.1 Verifizierte historische BTC-Signalpipeline

Der historische Builder
`archive/HISTORICAL_K3_K10_2026-01-06/scripts_legacy_from_root/build_price_data_with_signals.py`
erzeugte `data/price_data_with_signals.csv`.

Die zwölf vorhandenen binären Signalspalten wurden über 2.721.034 Zeilen gegen
die im Builder enthaltenen Regeln geprüft. Das Ergebnis waren null
Abweichungen. Der Builder ist damit als tatsächlicher Erzeuger dieser
Signalspalten empirisch bestätigt.

### 4.2 Verifizierte historische Regimelogik

`tools/add_regime.py` klassifizierte:

- `bull`, wenn `close > ma200`, `ema50 > ma200`, `roc > 0` und `adx >= 15`,
- `bear`, wenn `close < ma200`, `ema50 < ma200`, `roc < 0` und `adx >= 15`,
- andernfalls `side`.

Die vier Felder `market_regime`, `regime_signal`, `regime_bull` und
`regime_bear` wurden über 1.048.575 vorhandene Datenzeilen geprüft. Das Ergebnis
waren null Regelabweichungen.

### 4.3 Kritischer Integritätsbefund

Die Regime-Datei enthält exakt 1.048.575 Datenzeilen. Zusammen mit einer
Kopfzeile entspricht dies der maximalen Excel-Arbeitsblattgröße von 1.048.576
Zeilen. Da die vorgelagerte Signaldatei 2.721.034 Datenzeilen enthält, besteht
ein starkes Indiz, dass die Regime-Datei durch einen Spreadsheet-Roundtrip oder
einen vergleichbaren Zeilenlimit-Prozess abgeschnitten wurde.

Folgerungen:

- Die geprüften Regimeregeln sind innerhalb des vorhandenen Ausschnitts
  bestätigt.
- Die Regime-Datei darf nicht als vollständige historische Datenbasis gelten,
  bevor ihre Lineage geklärt oder sie reproduzierbar neu erzeugt wurde.
- Spreadsheet-Programme dürfen keine kanonische Transformationsstufe der
  RCC-002-Pipeline bilden.
- Zeilenzahl, Start-/Endzeit und zeitliche Kontinuität müssen an jeder
  Pipeline-Grenze geprüft werden.

### 4.4 Architektonische Legacy-Grenzen

Die historische Pipeline vermischte teilweise:

- Richtungssignale und Zustandsmerkmale,
- Regimeklassifikation und implizite Handelsfreigabe,
- Warm-up-Zeilen und scheinbar neutrale Werte,
- Forschungsdaten und dateispezifische Übergabeformate.

Die spätere GS-Generation führte kontinuierliche Signale und getrennte
Long-/Short-Gates ein, ist für BTC jedoch nicht vollständig durch einen
erhaltenen kanonischen Datensatz rekonstruierbar.

RCC-002 übernimmt daher keine Legacy-Pipeline unverändert. Verifizierte
Legacy-Logik wird als versionierbare Referenz erhalten, aber in klar getrennte
Stufen überführt.
...
Jede Pipeline-Stufe MUST genau eine fachliche Hauptverantwortung besitzen.
Insbesondere MUST Folgendes getrennt bleiben:

- Rohindikator und transformiertes Signal,
- Marktregime und Handels-Gate,
- Feature und zukunftsbezogenes Label,
- Datenberechnung und Strategieentscheidung,
- kanonischer Datensatz und nutzerspezifischer Analyseexport.

### 5.2 Determinismus

Identische Eingabebytes, identische Codeversion und identische Konfiguration
MUST byte-identische oder nach dokumentierter Serialisierungsnormalisierung
inhaltlich identische Ausgaben erzeugen.

Nichtdeterministische Operationen MUST entweder ausgeschlossen oder durch
explizite Seeds und dokumentierte Laufzeitbedingungen kontrolliert werden.

### 5.3 Point-in-Time-Korrektheit

Ein Feld der Zeile `t` MUST ausschließlich Informationen verwenden, die zum
definierten Entscheidungszeitpunkt von `t` verfügbar waren.

Zukunftsbezogene Werte dürfen ausschließlich in explizit als Label- oder
Forward-Return-Felder gekennzeichneten Spalten vorkommen. Diese Felder MUST für
Live- und Paper-Trading-Eingaben technisch ausgeschlossen werden können.

Insbesondere unzulässig sind in S0 bis S6:

- zentrierte Rolling Windows,
- rückwirkend berechnete Glättungen mit Zukunftswerten,
- Normalisierung gegen Statistiken des vollständigen Datensatzes,
- rückwärts gerichtetes Auffüllen aus späteren Beobachtungen,
- Regime- oder Gate-Revisionen anhand späterer Marktbewegungen,
- Auswahl von Parametern anhand des späteren Test- oder Live-Zeitraums.

### 5.4 Fail-Closed

Fehlende Pflichtspalten, ungültige Zeitachsen, Duplikate, unerwartete
Zeilenverluste, Schemaabweichungen oder Checksum-Fehler MUST den kanonischen
Build abbrechen.

Warnungen allein sind nur für ausdrücklich als nichtkritisch klassifizierte
Befunde zulässig.

### 5.5 Unveränderlichkeit und Lineage

Kanonische Eingabedateien MUST unverändert bleiben. Jede Transformation erzeugt
ein neues versioniertes Artefakt mit nachvollziehbarer Eltern-Kind-Beziehung.

Manuelle Änderungen an kanonischen CSV-, Parquet- oder Manifest-Dateien sind
unzulässig.

### 5.6 Konfigurierbarkeit ohne implizite Semantik

Parameter MUST in versionierten Konfigurationen liegen. Dateinamen,
Verzeichnisnamen oder Kommentare dürfen keine alleinige Quelle fachlicher
Semantik sein.

Jede Konfiguration MUST eine eindeutige Schema-, Feature-, Signal-, Regime-,
Gate- und Label-Version referenzieren.

### 5.7 Qualitätsflag-Propagation

Qualitätsinformationen MUST entlang der Pipeline erhalten bleiben. Eine
nachgelagerte Stufe darf einen ungültigen, synthetischen oder unvollständigen
Quellzustand nicht durch eine erfolgreiche Berechnung verdecken.

Jede abgeleitete Zeile MUST erkennen lassen, ob ihre Eingabefenster:

- ausschließlich beobachtete gültige Kerzen,
- mindestens eine synthetische Kerze,
- mindestens eine Lücke oder ungültige Kerze,
- unzureichende Warm-up-Historie

enthalten.

## 6. Kanonischer Datenfluss

Die verbindliche Stufenfolge lautet:

1. `S0_SOURCE`: unveränderte Quelldateien und Quellmetadaten.
2. `S1_NORMALIZED`: normalisierte, noch nicht lückenbehandelte OHLCV-Daten.
3. `S2_VALIDATED`: qualitätsgeprüfte kanonische Marktzeitreihe.
...
Keine Stufe darf eine nachgelagerte Stufe als Eingabe verwenden.

### 6.1 Verbindliche Stufenverträge

Jede Stufe MUST einen maschinenlesbaren Vertrag besitzen mit:

- akzeptierter Eingabeschemaversion,
- erzeugter Ausgabeschemaversion,
- Pflicht- und optionalen Feldern,
- Schlüssel- und Sortierregeln,
- erlaubten Zeilenzahlveränderungen,
- erlaubten Nullwerten,
- Eingangs- und Ausgangsinvarianten,
- Fehlerklassen und Exit-Codes,
- Komponenten-ID und Komponenten-Version.

Eine Stufe darf Eingaben mit unbekannter oder inkompatibler Schemaversion nicht
stillschweigend verarbeiten.
...
- Quelle, Markt, Symbol, Intervall und Datenart dokumentieren,
- Originaldateien unverändert speichern,
- kryptografische Checksummen erfassen,
- Abrufzeitpunkt und erwarteten Zeitraum dokumentieren,
- unvollständige Downloads erkennen,
- vorhandene Quellmetadaten erhalten.
...
- Werte runden,
- Duplikate still entfernen,
- Dateien über Spreadsheet-Software transformieren.
...
- einen kanonischen Zeitindex festlegen,
- numerische Datentypen explizit erzwingen,
- Spaltennamen auf das kanonische Schema abbilden,
- Sortierung deterministisch herstellen,
- Quellzeilen und Normalisierungsentscheidungen rückverfolgbar halten.

Bei Duplikaten oder kollidierenden Quellzeilen darf S1 keine Beobachtung
stillschweigend auswählen. Auflösungsregel, Priorität und verworfene
Quellreferenzen MUST dokumentiert werden; ohne freigegebene Regel bricht der
Build ab.

Das kanonische OHLCV-Mindestschema lautet:

| Feld | Bedeutung |
|---|---|
| `open_time` | Beginn der Kerze in UTC |
| `close_time` | Ende der Kerze in UTC, falls verfügbar oder deterministisch ableitbar |
| `open` | Eröffnungspreis |
| `high` | Höchstpreis |
| `low` | Tiefstpreis |
| `close` | Schlusskurs |
| `volume` | Basisasset-Volumen |
| `quote_volume` | Quote-Asset-Volumen, falls verfügbar |
| `trade_count` | Anzahl Trades, falls verfügbar |
| `source_id` | Referenz auf Quelle und Rohartefakt |

### 7.3 S2_VALIDATED – Validierung und kanonische Zeitreihe

S2 MUST mindestens prüfen:

- Zeitstempel-Parsing und UTC-Eindeutigkeit,
- exakte Intervallausrichtung,
- streng monotone Sortierung,
- doppelte Zeitstempel,
- fehlende erwartete Intervalle,
- nicht endliche numerische Werte,
- `high >= max(open, close, low)`,
- `low <= min(open, close, high)`,
- positive Preise,
- nichtnegative Volumina und Trade Counts,
- erwartete Start-/Endzeit,
- erwartete und tatsächliche Zeilenzahl.

Lückenbehandlung MUST konfigurationsgebunden und explizit sein.

Für das kanonische beobachtete S2-Artefakt sind ausschließlich folgende
Ergebnisse zulässig:

- Build-Abbruch,
- Beibehaltung der beobachteten Kerzen mit dokumentierter Lücke und
  Qualitätsflag.

Der kanonische beobachtete S2-Datensatz MUST ausschließlich beobachtete Kerzen
enthalten. Eine deterministisch synthetisch vervollständigte
Kontinuitätsansicht MAY nach separat freigegebener Regel als eigenes
nichtkanonisches Artefakt erzeugt werden.

Eine solche Ansicht MUST:

- eine eigene View-, Build- und Profil-ID besitzen,
- jede synthetische Kerze dauerhaft markieren,
- vom kanonischen beobachteten S2-Artefakt getrennt bleiben,
- aus kanonischen Returns und Labels standardmäßig ausgeschlossen werden,
- in einer separaten Sensitivitätsanalyse untersucht werden.

Synthetische Kerzen dürfen niemals stillschweigend wie beobachtete Marktdaten
behandelt werden.

Die erwartete Zeilenzahl MUST aus Startzeit, Endzeit und Intervall unabhängig
berechnet werden. Ein Vergleich lediglich mit der Zeilenzahl der vorherigen
Datei ist nicht ausreichend.
...
- Formel, Parameter, Eingabefelder und Warm-up definieren,
- numerische Bibliotheks- und Versionsabhängigkeiten dokumentieren,
- die Verfügbarkeit jeder Zeile über Validitätsmasken ausweisen,
- keine Warm-up-NaNs implizit als neutrales Signal interpretieren.

Rolling-, Expanding- und EWM-Berechnungen MUST ausdrücklich auf kausale,
rückwärtsgerichtete Fenster beschränkt sein. Jede Implementierung benötigt
Referenztests an kleinen, manuell oder unabhängig berechneten Testvektoren.

Die exakten Definitionen werden in
`RCC_002_INDICATOR_SPECIFICATION` festgelegt.

### 7.5 S4_SIGNALS – Signaltransformation

S4 transformiert Indikatorzustände in standardisierte Signalfelder.

RCC-002 MUST gleichzeitig unterstützen können:

- eine versionierte Legacy-Reproduktion binärer Signale,
- richtungssymmetrische oder asymmetrische diskrete Signale,
- kontinuierliche normierte Scores,
- explizite Neutral- und Invalid-Zustände.

Ein Signal MUST mindestens folgende Semantik unterscheiden:

- bullish,
- bearish,
- neutral,
- nicht verfügbar/ungültig.

`0` darf daher nicht gleichzeitig für „neutral“ und „nicht berechenbar“
verwendet werden.

Die exakten Transformationen werden in
`RCC_002_SIGNAL_TRANSFORMATION` festgelegt.

### 7.6 S5_REGIMES – Marktklassifikation

S5 beschreibt den Markt, nicht die Handelserlaubnis.

Jedes Regimemodell MUST:

- eine Modell-ID und Version besitzen,
- ausschließlich zeitpunktverfügbare Features verwenden,
- Zustände und Übergangsregeln vollständig definieren,
- Warm-up und Unbestimmtheit explizit ausweisen,
- optionale Konfidenz- oder Evidenzwerte getrennt speichern.
...
Zustandsbedingungen MAY nur durch ein explizit registriertes und versioniertes
Gate-Profil einfließen. Eine bloße Verfügbarkeit eines solchen Feldes erzeugt
keine implizite Gate-Bedingung.
...
Ein Gate darf die Marktregimebezeichnung nicht überschreiben. Mehrere
Gate-Gründe SHOULD als maschinenlesbare Codes und nicht nur als Freitext
gespeichert werden.

Ein gemeinsames Feld `gate_reason_codes` MAY nur zusätzlich erzeugt werden,
wenn seine Aggregationsregel registriert, versioniert und eindeutig ist.
...
S7 ist die einzige reguläre Stufe, die zukünftige Marktwerte neu berechnen
darf. S8 darf diese Werte ausschließlich unverändert in ausdrücklich dafür
vorgesehene Research- oder ML-Views übernehmen.

Jedes Forward-Return- oder Label-Feld MUST:

- den Horizont und die Preisreferenz im Namen oder Schema ausweisen,
- Long-/Short-Semantik eindeutig definieren,
- Randzeilen ohne vollständigen Zukunftshorizont als ungültig markieren,
- Gebühren- und Slippageannahmen getrennt von reinen Marktreturns halten,
- bei überlappenden Horizonten die statistische Abhängigkeit dokumentieren.

Label-Felder MUST in Live-/Paper-Datensätzen vollständig entfernbar sein.

### 7.9 S8_EXPORT – Konsumartefakte

S8 MAY mehrere Views aus demselben kanonischen Build erzeugen, zum Beispiel:

- Research Feature Dataset,
- Backtest Dataset,
- Regime Analysis Dataset,
- ML Training Dataset,
- Paper-/Live-Parity Dataset ohne Zukunftsfelder.

Jeder Export MUST auf dieselbe Build-ID und dieselben vorgelagerten Artefakte
zurückführbar sein.

Jeder S8-View MUST eine explizite Allowlist seiner Felder besitzen.
Blocklisten allein sind für den Ausschluss von Zukunftsfeldern nicht
ausreichend.
...
Parquet SHOULD das kanonische interne Tabellenformat sein, sofern die
Implementierungsprüfung keine begründete Inkompatibilität zeigt.

CSV MAY als portabler Analyseexport erzeugt werden, darf jedoch nicht alleinige
kanonische Quelle sein, weil Datentypen, Zeitzonen, Nullwerte und numerische
Präzision schwächer abgesichert sind.

XLSX, ODS und andere Spreadsheet-Formate MUST NOT als kanonische Zwischen- oder
Ausgabeformate verwendet werden.

### 8.2 Partitionierung

Große Datensätze SHOULD nach stabilen fachlichen Schlüsseln partitioniert
werden, vorzugsweise:

- Symbol,
- Intervall,
- Kalenderjahr oder Kalenderjahr/-monat.

Partitionierung darf die globale Zeitachsenprüfung nicht ersetzen.

## 9. Schema- und Namensregeln

Jede Spalte MUST genau einer Feldklasse angehören:

- `source_*`,
- OHLCV-Basisfeld,
- `quality_*`,
- `indicator_*` oder registrierter Indikatorname,
- `signal_*`,
- `regime_*`,
- `gate_*`,
- `fwd_*`,
...
S7 verwendet verbindlich `fwd_*` für Forward Returns und Forward Excursions.
`forward_*` ist kein paralleles Synonym.

Semantisch unterschiedliche Werte dürfen keine identischen Spaltennamen tragen.
Spaltenumbenennungen gelten als Schemaänderung und benötigen eine neue
Schemaversion oder dokumentierte Migration.

## 10. Warm-up, Nullwerte und Gültigkeitsmasken

Für jede berechnete Feldgruppe MUST definiert werden:

- minimale Historienlänge,
- erste gültige Zeile,
- Verhalten bei Quelllücken,
- Verhalten nach synthetischen Kerzen,
- `NaN`-/Null-Semantik,
- zugehörige Gültigkeitsmaske.

RCC-002 MUST verhindern, dass ein Vergleich mit `NaN` automatisch ein fachlich
neutrales oder negatives Signal erzeugt.

Ein Gesamtdatensatz MAY Warm-up-Zeilen behalten, doch konsumierende Komponenten
MUST sie anhand expliziter Validitätsfelder zuverlässig ausschließen können.

## 11. Manifest und Provenienz

Jeder Build MUST ein maschinenlesbares Manifest erzeugen. Das Manifest enthält
mindestens:
...
- Code-Repository und Commit-Hash,
- Status des Git-Worktrees,
- Python- und Bibliotheksversionen,
- Konfigurationsdatei und Konfigurationschecksumme,
- Schema- und Komponenten-Versionen,
- Startzeit, Endzeit und Zeilenzahl je Stufe,
- Anzahl Duplikate, Lücken, synthetische Kerzen und verworfene Zeilen,
- Artefaktpfade, Dateigrößen und Checksummen,
- Parent-/Child-Lineage,
- Validierungsergebnis,
- bekannte Abweichungen oder genehmigte Ausnahmen.

Ein Build ohne vollständiges Manifest darf nicht als kanonisch markiert werden.

### 11.1 Quellkorrekturen und Revisionshistorie

Werden historische Quelldaten durch den Anbieter korrigiert oder nachträglich
ergänzt, MUST ein neuer Source-Snapshot und ein neuer Build erzeugt werden.
Bestehende kanonische Builds bleiben unverändert.

Das Manifest MUST dann zusätzlich dokumentieren:

- ersetzte Source-Snapshot-ID,
- betroffene Zeitbereiche,
- Anzahl und Art geänderter Beobachtungen,
- Auswirkung auf nachgelagerte Artefakte,
- Grund der Neubildung.

## 12. Qualitäts-Gates

Ein RCC-002-Build darf nach Abschluss von S8 nur dann als kanonisch
...
8. Stichproben gegen unabhängige Referenzberechnungen bestanden wurden,
9. alle S8-Views ihre Feld- und Leakage-Allowlist bestanden haben.

Jedes Gate MUST `PASS`, `FAIL` oder `NOT_APPLICABLE` mit Begründung ausgeben.
Ein kritisches `FAIL` blockiert die Veröffentlichung.

## 13. Reproduzierbarkeitsprüfungen

Vor der Freigabe MUST mindestens geprüft werden:

- Wiederholungsbuild auf derselben Umgebung,
- Wiederholungsbuild auf einem zweiten Gerät oder einer isolierten Umgebung,
- Vergleich von Schema, Zeilenzahlen, Zeitgrenzen und Checksummen,
- deterministische Stichprobe einzelner Indikatorwerte,
- vollständige Legacy-Reproduktion für ausdrücklich unterstützte Legacy-Modi,
- Ausschluss von Forward-/Label-Spalten aus Live-Paritätsviews.

Abweichungen durch zulässige Float-Repräsentation benötigen dokumentierte
Toleranzen pro Feld; pauschale globale Toleranzen sind nicht zulässig.

## 14. Fehler- und Wiederanlaufverhalten

Jede Stufe MUST:

- atomar oder über temporäre Artefakte schreiben,
- unvollständige Ausgaben eindeutig markieren,
- bereits vorhandene kanonische Artefakte nicht still überschreiben,
- einen eindeutigen Exit-Code liefern,
- strukturierte Fehlerberichte erzeugen,
- nach Fehlerbehebung deterministisch neu startbar sein.

Ein fehlgeschlagener Build darf keine Datei als erfolgreich oder kanonisch
registrieren.

## 15. Forschungs- und Produktionsparität

Research, Backtest, Paper und Live SHOULD dieselben freigegebenen
Implementierungen für zeitpunktverfügbare Felder verwenden.

Unterschiede dürfen nur durch explizite Views oder Adapter entstehen:
...
- Paper und Live müssen identische Feature-, Signal-, Regime- und Gate-Versionen
  verwenden, sofern kein dokumentierter A/B-Test vorliegt.

## 16. Erweiterbarkeit

Weitere Assets, Märkte oder Intervalle MAY integriert werden, wenn:

- ihr Quellschema registriert ist,
- Asset-/Marktbesonderheiten dokumentiert sind,
- Zeitachsen- und Qualitätsregeln bestanden werden,
- keine stillschweigende Übertragung BTC-spezifischer Schwellenwerte erfolgt,
- Transfer- und Robustheitsprüfungen separat durchgeführt werden.

Cross-Market-Features MUST den Veröffentlichungszeitpunkt und mögliche
Zeitversätze jeder Quelle berücksichtigen.

## 17. Sicherheits- und Governance-Regeln

- Kanonische Builds MUST in versionierten, schreibgeschützten Zielpfaden
  veröffentlicht werden.
- Secrets und API-Schlüssel dürfen weder in Datensätzen noch Manifesten
  enthalten sein.
- Externe Datenquellen und Lizenzen MUST dokumentiert werden.
- Jede fachliche Regeländerung benötigt eine Versionsänderung und einen
  Vergleichsbuild.
- Eine Änderung pro Validierungsexperiment bleibt der Standard.
- Code ist die ausführbare Quelle der Wahrheit; Spezifikation und Code müssen
  vor Freigabe durch Konformitätstests abgeglichen werden.

## 18. Abnahmekriterien für RCC-002

RCC-002 gilt erst als freigabefähig, wenn:

1. alle nachgeordneten Spezifikationen vollständig sind,
2. interne Konsistenzprüfung und Scientific Consistency Review bestanden sind,
3. Architektur- und Schnittstellenreview bestanden sind,
4. Implementierung alle normativen Anforderungen abdeckt,
5. vollständiger BTCUSDT-1m-Build auf der Workstation erfolgreich ist,
6. keine unerklärten Lücken, Duplikate oder Zeilenverluste bestehen,
7. Rebuild und Checksummenprüfung erfolgreich sind,
8. Research-/Live-Leakage-Test bestanden ist,
9. Legacy-Vergleiche dokumentiert sind,
10. Manifest, Dataset Lineage und Knowledge Lineage vollständig sind,
11. Quellkorrektur- und Rebuild-Verfahren getestet sind,
12. Feld-Allowlists für alle veröffentlichten Views validiert sind.

## 19. Nachgeordnete Spezifikationen

Folgende Dokumente bilden die normativen Ergänzungen:

1. `RCC_002_DATA_VALIDATION`
2. `RCC_002_INDICATOR_SPECIFICATION`
3. `RCC_002_SIGNAL_TRANSFORMATION`
4. `RCC_002_REGIME_AND_GATE_SPECIFICATION`
5. `RCC_002_LABEL_AND_FORWARD_RETURN_SPECIFICATION`
6. `RCC_002_REPRODUCIBILITY_AND_MANIFEST`

Bei Konflikten gilt:

1. eine explizit freigegebene neuere Spezifikation,
2. diese übergeordnete RCC-002-Spezifikation,
3. historische Builder nur als Legacy-Referenz,
4. Kommentare oder Dateinamen niemals als alleinige Norm.

## 20. Offene Entscheidungen

Vor Status `Approved for Implementation` bleiben mindestens folgende
Implementierungsparameter festzulegen:

- konkretes Parquet-Writerprofil und Kompressionsverfahren,
...
Korrekturen der abhängigen Spezifikationen als geschlossen. Er identifizierte
für dieses Dokument:
...
Sie ist noch nicht zur Implementierung freigegeben. Der Status bleibt bis zum
bestandenen SCR-Re-Review:

`SCR-Corrected Draft – Re-Review Pending`.

Nächste vorgeschriebene Schritte:

1. Korrektur der Reproducibility and Manifest Specification auf Version 0.3.0
   für SCR-RR-B02.
2. Aktualisierung betroffener Abhängigkeitsversionen und Review Evidence.
3. Neues gemeinsames Spezifikationspaket aus allen sieben aktuellen
   Dokumenten.
4. Fokussierter Scientific Consistency Re-Review von SCR-RR-B01,
   SCR-RR-B02 und SCR-RR-m01 sowie Prüfung auf neu entstandene
   Querwidersprüche.
5. Editorial Pass.
6. Architecture Integrity Review.
7. Unabhängige Reviews durch Gemini und Claude.
8. Certification und Freigabe zur Implementierung.
...
| Version | 0.1.0 |
| Datum | 2026-07-23 |
| Status | Review-Ready Draft – Internal Review Completed |
| Übergeordnetes Dokument | `RCC_002_DATA_PIPELINE_SPECIFICATION_2026-07-23.md`, Version 0.2.0 |
...
| Scientific Consistency Review | Ausstehend | Erfolgt gemeinsam mit der vollständigen RCC-002-Spezifikationsfamilie |
...
„reparieren“. Stattdessen werden:

- Originalartefakt,
- rekonstruierter Legacy-Build,
- neuer RCC-002-Build

getrennt erhalten und vergleichend dokumentiert.
...
Sie ist noch nicht wissenschaftlich zertifiziert. Die endgültige Freigabe
erfolgt gemeinsam mit der vollständigen RCC-002-Spezifikationsfamilie.

Nächster fachlicher Schritt:

`RCC_002_INDICATOR_SPECIFICATION`
...
| Status | Review-Ready Draft – Internal Review Completed |
| Übergeordnetes Dokument | `RCC_002_DATA_PIPELINE_SPECIFICATION_2026-07-23.md`, Version 0.2.0 |
| Direkte Abhängigkeit | `RCC_002_DATA_VALIDATION_2026-07-23.md`, Version 0.1.0 |
...
| Scientific Consistency Review | Ausstehend | Erfolgt gemeinsam mit der RCC-002-Spezifikationsfamilie |
...
| Status | Review-Ready Draft – Internal Review Completed |
| Übergeordnetes Dokument | `RCC_002_DATA_PIPELINE_SPECIFICATION_2026-07-23.md`, Version 0.2.0 |
| Direkte Abhängigkeiten | `RCC_002_DATA_VALIDATION_2026-07-23.md`, Version 0.1.0; `RCC_002_INDICATOR_SPECIFICATION_2026-07-23.md`, Version 0.1.0 |
...
| Datum | 2026-07-23 |
| Status | SCR-Corrected Draft – Re-Review Pending |
| Übergeordnetes Dokument | `RCC_002_DATA_PIPELINE_SPECIFICATION_2026-07-23.md`, Version 0.3.0 |
| Direkte Abhängigkeiten | `RCC_002_DATA_VALIDATION_2026-07-23.md`, Version 0.1.0; `RCC_002_INDICATOR_SPECIFICATION_2026-07-23.md`, Version 0.1.0; `RCC_002_SIGNAL_TRANSFORMATION_2026-07-23.md`, Version 0.1.0 |
...
| Scientific Consistency Review | Nicht bestanden; Korrektur eingearbeitet | SCR-E01 und SCR-E02 aus `RCC-002-SCR-001` in Version 0.2.0 korrigiert; Re-Review ausstehend |
| Externe KI-Reviews | Ausstehend | Gemini und Claude werden in der späteren unabhängigen Review-Stufe eingesetzt |
...
- unabhängige Reviews mit Gemini und Claude,
...
Sie ist noch nicht wissenschaftlich zertifiziert. Der Status bleibt bis zum
bestandenen SCR-Re-Review:
...
`RCC_002_REPRODUCIBILITY_AND_MANIFEST`, Version 0.2.0,
vollständig korrigieren und anschließend den gemeinsamen SCR-Re-Review
durchführen.
...
| Status | Review-Ready Draft – Internal Review Completed |
| Übergeordnetes Dokument | `RCC_002_DATA_PIPELINE_SPECIFICATION_2026-07-23.md`, Version 0.2.0 |
| Direkte Abhängigkeiten | `RCC_002_DATA_VALIDATION_2026-07-23.md`, Version 0.1.0; `RCC_002_INDICATOR_SPECIFICATION_2026-07-23.md`, Version 0.1.0; `RCC_002_SIGNAL_TRANSFORMATION_2026-07-23.md`, Version 0.1.0; `RCC_002_REGIME_AND_GATE_SPECIFICATION_2026-07-23.md`, Version 0.1.0 |
...
`RCC_002_REPRODUCIBILITY_AND_MANIFEST`
...
| Primäre Abhängigkeit | `RCC_002_DATA_PIPELINE_SPECIFICATION_2026-07-23.md`, Version `0.4.0` |
| Fachliche Abhängigkeiten | `RCC_002_DATA_VALIDATION_2026-07-23.md`; `RCC_002_INDICATOR_SPECIFICATION_2026-07-23.md`; `RCC_002_SIGNAL_TRANSFORMATION_2026-07-23.md`; `RCC_002_REGIME_AND_GATE_SPECIFICATION_2026-07-23.md`, Version `0.2.0`; `RCC_002_LABEL_AND_FORWARD_RETURN_SPECIFICATION_2026-07-23.md` |
| Referenziert durch | RCC-002-Implementierungsplan; RCC-002-Build- und Prüfwerkzeuge; RCC-002-Dataset-Release-Dokumentation |
| Vorgesehene Reviews | Editorial Pass; Scientific Consistency Review; Architecture Integrity Review; unabhängige Reviews durch Gemini und Claude; Certification |
...
- Architecture Integrity Review;
- unabhängiges Gemini-Review;
- unabhängiges Claude-Review;
- Certification und Release-Freigabe.
...
{
  "review_id": "string",
  "review_type": "internal|editorial|scr|architecture|external",
  "reviewer": "string",
...
Gemini- und Claude-Reviews werden erst eingetragen, nachdem sie tatsächlich durchgeführt wurden.
...
2. Internal Review;
3. Editorial Pass;
4. Scientific Consistency Review;
5. Architecture Integrity Review;
6. unabhängige Reviews durch Gemini und Claude;
7. Certification;
8. Release- und Implementierungsfreigabe.
...
14. Gemini- und Claude-Reviewstatus nur anhand realer Reviews aktualisiert werden;
...
- behauptet keine bereits durchgeführten Gemini- oder Claude-Reviews.
...
Diese Version korrigiert die dem Dokument zugeordneten Befunde aus
`RCC-002-SCR-001` sowie `SCR-RR-B02` aus `RCC-002-SCR-002`. Sie gilt bis zum
erneuten fokussierten SCR-Re-Review als
`SCR-Corrected Draft – Re-Review Pending` und begründet noch keine
wissenschaftliche Freigabe oder Certification.
