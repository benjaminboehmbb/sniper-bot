...
#
# Build the GS-contract-compliant LONG FINAL for K12 (structural endpoint).
...
#
# We write:
#   1) results/GS/k12_long/strategy_results_GS_k12_long_FULL_CANONICAL_<ts>.csv  (N=1)
#   2) strategies/GS/LONG_FINAL_CANONICAL/strategies_GS_k12_long_FINAL_CANONICAL_<ts>.csv (N=1)
#
# No existing FINAL is overwritten. ASCII-only logs.
...
OUT_RESULTS_DIR = "results/GS/k12_long"
OUT_STRATEGIES_DIR = "strategies/GS/LONG_FINAL_CANONICAL"
...
def main():
    ...
    os.makedirs(OUT_STRATEGIES_DIR, exist_ok=True)
    ...
    for off in OFFSETS:
        ...
        # also keep additional GS outputs (if present) for transparency
    ...
    out_strat = os.path.join(
        OUT_STRATEGIES_DIR,
        f"strategies_GS_k12_long_FINAL_CANONICAL_{ts}.csv"
    )
    pd.DataFrame(
        [{
            "k": 12,
            "direction": "long",
            "source": "GS_structural_endpoint_canonical",
            "signals_key": res["signals_key"],
            "combination": res["combination"],
        }]
    ).to_csv(out_strat, index=False)

    print("[ok] wrote results:", out_results)
    print("[ok] wrote strategy:", out_strat)
    print("[ok] rows:", 1)
