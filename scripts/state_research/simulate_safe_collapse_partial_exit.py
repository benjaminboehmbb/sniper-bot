...
def detect_safe_collapse(group: pd.DataFrame) -> tuple[bool, int | None]:
...
def main() -> None:
    ...
    for key, group in shadow_df.groupby(["entry_timestamp_utc", "side"]):
        triggered, trigger_idx = detect_safe_collapse(group)
    ...
    summary_df = pd.DataFrame(
        [
            {
                "gate_name": "safe_collapse_partial_exit",
    ...
    detail_path = out_dir / f"safe_collapse_partial_exit_detail_{args.label}.csv"
    summary_path = out_dir / f"safe_collapse_partial_exit_summary_{args.label}.csv"

    detail_df.to_csv(detail_path, index=False)
    summary_df.to_csv(summary_path, index=False)

    print("OK: STEP13D safe collapse partial-exit simulation written")
    print(f"detail: {detail_path}")
    print(f"summary: {summary_path}")
    print()
    print("SAFE COLLAPSE PARTIAL EXIT SUMMARY")
