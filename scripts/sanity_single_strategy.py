...
def main():
    ...
    # Normierung & einfache Signale
    # RSI: Long-Signal, wenn RSI > 55; skaliert um 50 → [-50..+50] / 50 => [-1..+1]
    df["sig_rsi"] = ((df["rsi"] - 50.0) / 50.0).clip(-1, 1)
    # MACD: Vorzeichen von MACD-Linie vs. Signal
    df["sig_macd"] = np.sign(df["macd"] - df["macd_sig"])
    # Bollinger-Bandbreite als Trend-Stärke-Proxy (höher = mehr Trend), zentriert um Median
    w_med = df["bb_width"].rolling(200).median()
    df["sig_bb"] = np.sign((df["bb_width"] - w_med).fillna(0))

    # Gewichte (einfaches Beispiel)
    w_rsi, w_macd, w_bb = 0.5, 0.4, 0.1
    df["combo"] = w_rsi*df["sig_rsi"] + w_macd*df["sig_macd"] + w_bb*df["sig_bb"]

    # Regeln (long-only, simpel):
    ...
    trades_path = os.path.join(RES_DIR, "sanity_trades.csv")
    ...
    res = {
        ...
        "entry_rule": "combo>0.5; exit combo<=0; long-only",
        "outputs": {"trades_csv": os.path.relpath(trades_path, ROOT)}
    }
    with open(os.path.join(RES_DIR, "sanity_result.json"), "w", encoding="utf-8") as f:
        json.dump(res, f, ensure_ascii=False, indent=2)

    print("✅ Sanity-Backtest fertig.")
