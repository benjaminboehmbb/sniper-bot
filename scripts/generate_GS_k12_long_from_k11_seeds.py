...
#
# Gold-Standard structure expansion (UNWEIGHTED) for K12 LONG:
# - Reads K12 LONG seeds CSV (Top-250 selected from K11 FULL results)
# - Expands by adding exactly one new signal (weight=1.0) not already present
# - Produces unique K12 candidate combinations (stringified dict in column "combination")
# - Deterministic: stable sorting + canonical formatting + dedup on canonical key
# - Writes timestamped output CSV in strategies/GS/k12_long/
#
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Generate GS K12 LONG candidates from K11-selected seeds (unweighted structure expansion)."
    )
    ap.add_argument(
        "--seeds_csv",
        default="strategies/GS/k12_long/strategies_GS_k12_long_seeds_top250_minRoiP25-2.82_2026-01-09_15-37-30.csv",
    ...
    ap.add_argument(
        "--source",
        default="k11_seed_expansion_unweighted",
...
def main():
    ...
    out_name = f"strategies_GS_k12_long_from_k11_top250_unweighted_{ts}.csv"
