...
"""
Analyze K7 strategies (LONG/SHORT) - Windows spawn safe.

Fixes:
- Force repo root into sys.path (spawn-safe).
- Multiprocessing initializer/worker are TOP-LEVEL (pickleable on Windows).
- Evaluator signature: evaluate_strategy(price_df, comb, direction)

Usage (Windows PowerShell, repo root):
  .\.venv\Scripts\python.exe scripts\analyze_k7_long.py --input-glob "data\strategies_....csv" --price-csv "data\price_data_with_signals.csv" --direction long --num-procs 2 --limit 10000
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Analyze K7 strategies.")
    ...
    p.add_argument("--out", default=f"results{ksep()}k7_long{ksep()}strategy_results_k7_long_{utc_ts_compact()}.csv", help="Output CSV path")
