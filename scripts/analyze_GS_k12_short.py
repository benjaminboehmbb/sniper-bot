...
#
# Gold-Standard analyzer for K12 SHORT using:
#   engine.simtraderGS.evaluate_strategy(price_df, comb, direction)
#
# Key properties (must match GS pipeline):
# - Window slicing: rows=200000 over offsets: 0,500000,1000000,1500000
# - Fee external: roi_fee = roi - fee * trades
# - Robustness metric: roi_fee_p25 across offsets (not min)
# - Deterministic: no randomness; .copy() on window slices
# - ASCII-only prints
#
# SMOKE (N=1):
#   python3 scripts/analyze_GS_k12_short.py --strategies_csv <path> --limit 1
# FULL (also N=1 here):
#   python3 scripts/analyze_GS_k12_short.py --strategies_csv <path>
#
# Default price CSV:
...
def parse_args():
    ap = argparse.ArgumentParser(description="Analyze GS K12 SHORT (GS windows+offsets, external fee, roi_fee_p25).")
    ap.add_argument(
        "--csv",
        default="data/btcusdt_1m_2026-01-07/simtraderGS/"
                "btcusdt_1m_price_2017_2025_GS_PLUS_FORWARD_WITH_SIGNALS_REGIMEV1_ASYMGATE.csv",
        help="Price CSV (GS, fixed)",
    )
    ap.add_argument(
        "--strategies_csv",
        required=True,
        help="Strategies CSV (must contain column 'combination')",
    )
    ap.add_argument("--rows", type=int, default=200000, help="Window size per offset (fixed by GS)")
    ap.add_argument("--offsets", default=OFFSETS_DEFAULT, help="Comma-separated offsets (fixed by GS)")
    ...
    ap.add_argument("--limit", type=int, default=0, help="If >0: analyze only first N strategies (SMOKE)")
    ap.add_argument("--out_dir", default="results/GS/k12_short")
...
def normalize_engine_output(out):
    # Support dict or (roi, trades) tuple/list.
...
def main():
    ...
    out_path = os.path.join(args.out_dir, f"strategy_results_GS_k12_short_{mode}_{ts}.csv")

    max_end = max(offsets) + int(args.rows)
    if max_end > len(price_df):
        raise RuntimeError(
            "Requested window exceeds price_df length: need_end=%d len=%d"
            % (max_end, len(price_df))
        )
    ...
    for i, row in enumerate(strat_df.itertuples(index=False), start=1):
        ...
        for off in offsets:
            ...
            rec["roi_off_%d" % off] = roi
            rec["trades_off_%d" % off] = trades
            rec["roi_fee_off_%d" % off] = roi_fee
    ...
    per_off = []
    for off in offsets:
        per_off.extend(["roi_off_%d" % off, "trades_off_%d" % off, "roi_fee_off_%d" % off])
