...
def import_evaluator():
    ...
    # 2) file-based imports (covers missing engine/__init__.py etc.)
    candidates = [
...
def init_worker(price_path: str):
    ...
    if "open_time" not in cols and "timestamp" not in cols and "datetime" not in cols:
        print("[warn] price data missing open_time/timestamp/datetime (check if expected).")
...
def main():
    ...
    ap.add_argument("--limit", type=int, default=0,
                    help="If >0, only analyze first N strategies (for smoke/intermediate tests).")
    args = ap.parse_args()
