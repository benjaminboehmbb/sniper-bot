...
#
# GS analyzer for K11 SHORT using engine.simtraderGS.evaluate_strategy(price_df, comb, direction)
...
def parse_args():
    ap = argparse.ArgumentParser(description="Analyze GS K11 SHORT candidates (slice offsets/rows; evaluate_strategy signature).")
    ap.add_argument(
        "--csv",
    ...
    ap.add_argument(
        "--strategies_csv",
        required=True,
        help="Strategies CSV (K11 SHORT candidates)",
    )
    ...
    ap.add_argument("--out_dir", default="results/GS/k11_short")
...
def main():
    ...
    out_path = os.path.join(args.out_dir, f"strategy_results_GS_k11_short_{mode}_{ts}.csv")
