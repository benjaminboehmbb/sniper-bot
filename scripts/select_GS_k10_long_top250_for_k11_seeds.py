...
# - Input:  K10 LONG FULL results CSV (roi_fee_* already fee-adjusted externally)
# - Robustness metric: roi_fee_p25 over offsets (25th percentile across roi_fee_off_*)
# - Gate (K10->K11): roi_fee_p25 >= -2.65 (default; calibrated from K10 FULL)
# - Rank: roi_fee_p25 desc, then roi_fee_mean desc
# - Output: K11 LONG seeds CSV (Top-N, default 250) in strategies/GS/k11_long/
#
# Properties:
# - Deterministic: stable sorting, canonical signals_key
# - Schema-robust: overwrites k/direction/source if they already exist
# - ASCII-only prints
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Select K11 LONG seeds from K10 LONG FULL GS results (robust roi_fee_p25 gate)."
    )
    ap.add_argument(
        "--in_results",
        default="results/GS/k10_long/strategy_results_GS_k10_long_FULL_2026-01-09_14-46-30.csv",
        help="Input K10 LONG FULL results CSV",
    )
    ap.add_argument(
        "--out_dir",
        default="strategies/GS/k11_long",
        help="Output directory for K11 LONG seeds CSV",
    )
    ap.add_argument(
        "--top_n",
        type=int,
    ...
    ap.add_argument(
        ...
        type=int,
        default=11,
        help="Write this K into output column 'k' (K11 seeds -> 11)",
    )
    ap.add_argument(
        "--direction",
        default="long",
...
def main() -> None:
    ...
    out_name = (
        f"strategies_GS_k11_long_seeds_top{args.top_n}"
