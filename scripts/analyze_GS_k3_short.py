...
#
# GS analyzer for K3 SHORT using engine.simtraderGS.evaluate_strategy(price_df, comb, direction)
...
#
# SMOKE:
#   python3 scripts/analyze_GS_k3_short.py --strategies_csv <path> --limit 30
# FULL:
#   python3 scripts/analyze_GS_k3_short.py --strategies_csv <path>
...
def parse_args():
    ap = argparse.ArgumentParser(description="Analyze GS K3 SHORT candidates (slice offsets/rows; evaluate_strategy signature).")
    ap.add_argument(
        "--csv",
    ...
    ap.add_argument(
        "--strategies_csv",
        required=True,
        help="Strategies CSV (K3 SHORT candidates)",
    )
    ...
    ap.add_argument("--out_dir", default="results/GS/k3_short")
...
def main():
    ...
    out_path = os.path.join(args.out_dir, f"strategy_results_GS_k3_short_{mode}_{ts}.csv")
