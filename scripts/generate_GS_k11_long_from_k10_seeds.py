...
#
# Gold-Standard structure expansion (UNWEIGHTED) for K11 LONG:
# - Reads K11 LONG seeds CSV (Top-250 selected from K10 FULL results)
# - Expands by adding exactly one new signal (weight=1.0) not already present
# - Produces unique K11 candidate combinations (stringified dict in column "combination")
# - Deterministic: stable sorting + canonical formatting + dedup on canonical key
# - Writes timestamped output CSV in strategies/GS/k11_long/
#
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Generate GS K11 LONG candidates from K10-selected seeds (unweighted structure expansion)."
    )
    ap.add_argument(
        "--seeds_csv",
        default="strategies/GS/k11_long/strategies_GS_k11_long_seeds_top250_minRoiP25-2.69_2026-01-09_15-13-56.csv",
    ...
    ap.add_argument(
        "--source",
        default="k10_seed_expansion_unweighted",
...
def main():
    ...
    # Deterministic seed order: prefer robust ranking columns if present
    sort_cols = []
    ...
    out_name = f"strategies_GS_k11_long_from_k10_top250_unweighted_{ts}.csv"
