...
#
# GS analyzer for K10 LONG using engine.simtraderGS.evaluate_strategy(price_df, comb, direction)
# - Offsets/rows implemented via slicing price_df
# - Fee external: roi_fee = roi - fee * trades
# - ASCII-only prints
#
# SMOKE:
#   python3 scripts/analyze_GS_k10_long.py --limit 500
# FULL:
#   python3 scripts/analyze_GS_k10_long.py
...
def parse_args():
    ap = argparse.ArgumentParser(description="Analyze GS K10 LONG candidates (slice offsets/rows; evaluate_strategy signature).")
    ap.add_argument(
        "--csv",
    ...
    ap.add_argument(
        "--strategies_csv",
        default="strategies/GS/k10_long/strategies_GS_k10_long_from_k9_top250_unweighted_2026-01-09_14-36-41.csv",
    ...
    ap.add_argument("--out_dir", default="results/GS/k10_long")
...
def main():
    ...
    out_path = os.path.join(args.out_dir, f"strategy_results_GS_k10_long_{mode}_{ts}.csv")
