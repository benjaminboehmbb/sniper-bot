...
#
# GS analyzer for K9 LONG using engine.simtraderGS.evaluate_strategy(price_df, comb, direction)
# - Offsets/rows implemented via slicing price_df (because engine signature is minimal)
# - Fee external: roi_fee = roi - fee * trades
# - ASCII-only prints
#
# SMOKE:
#   python3 scripts/analyze_GS_k9_long.py --limit 500
# FULL:
#   python3 scripts/analyze_GS_k9_long.py
...
def parse_args():
    ap = argparse.ArgumentParser(description="Analyze GS K9 LONG candidates (slice offsets/rows; evaluate_strategy signature).")
    ap.add_argument(
        "--csv",
    ...
    ap.add_argument(
        "--strategies_csv",
        default="strategies/GS/k9_long/strategies_GS_k9_long_from_k8_top250_unweighted_2026-01-09_13-30-58.csv",
        help="Strategies CSV (candidates)",
    )
    ...
    ap.add_argument("--out_dir", default="results/GS/k9_long")
    ap.add_argument("--progress_step", type=int, default=50)
...
def normalize_engine_output(out):
    """
    Accept dict or (roi, trades) tuple/list.
    Expected keys for dict: roi, num_trades (fallback: trades, n_trades).
    """
...
def main():
    ...
    out_path = os.path.join(args.out_dir, f"strategy_results_GS_k9_long_{mode}_{ts}.csv")
    ...
    print("[info] ENGINE: module_function:evaluate_strategy(price_df, comb, direction)")
    print("[info] N:", n_total)

    results = []

    # Precompute max needed slice end
    max_end = max(offsets) + int(args.rows)
    if max_end > len(price_df):
        raise RuntimeError(
            f"Requested window exceeds price_df length: need_end={max_end}, len={len(price_df)}"
        )
    ...
    # Column order
    core = ["combination", "signals_key", "trades_sum", "roi_mean", "roi_fee_mean"]
    per_off = []
    for off in offsets:
