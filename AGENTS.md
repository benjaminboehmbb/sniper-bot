...
6. Validation after implementation.
   After every script creation or code change, execute the appropriate focused functional tests and applicable regression tests before considering the work complete. Report the exact commands and results. Never describe an implementation as released, approved or complete while required tests are missing or failing.
...
10. Established code requires evidence before modification.
    Do not modify established runtime scripts merely because an alternative implementation appears easier. First identify the active runtime boundary, the governing design or specification, and the concrete need for the change. Files explicitly marked frozen or read-only remain untouched. When the authority or intended file scope is genuinely unclear, stop and ask the user before editing.

11. Minimal and testable changes.
    Prefer the smallest change that satisfies the approved requirement. Do not create sidecars, parallel modules or replacement implementations unless they are justified by the existing architecture. Test a new module itself; passing tests for adjacent modules do not validate an untested new script.

12. WSL workspace discipline.
    For the Windows desktop setup, use the WSL-native agent for this repository and keep the canonical checkout in the Linux home filesystem. Before mutations, verify that the working directory and Git root resolve to the same WSL path. Do not treat a Windows app-resource path or a `\\wsl.localhost` path embedded below a Windows application directory as the canonical checkout.

13. No assumption when consequences are material.
    Ask the user when missing information would materially change architecture, scientific methodology, destructive cleanup, release status or the files authorized for modification. Continue autonomously only when repository evidence and the requested scope make the action unambiguous.
